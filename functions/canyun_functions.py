...
#     print('check')
    # universal.objective = BM_rxn # Set objective to max BM production
...
def make_canyun2(model, dgfba_expr, well_id_media_dict, objective_rxn_id = 'Growth'):
...
#                 print(key, 'Growth', solution.objective_value, solution.status)
            elif abs(solution.objective_value) < 0.01 and solution.status != 'infeasible':
                no_comp_growth.append(key)
#                 print(key, 'No Growth', solution.objective_value, solution.status)
            elif solution.status == 'infeasible':
                infeasible_list.append(key)
#                 print(key, 'Infeasible', solution.objective_value, solution.status)
        except:
...
def make_usage_dict2(model, solution_dict, gng_dict_long):
    usage_dict = {}
    count = 0
...
#                 if not rxn_id.startswith('EX') and not rxn_id.startswith('SNK') and not rxn_id.startswith('Growth') and not rxn_id.startswith('Biomass'):
